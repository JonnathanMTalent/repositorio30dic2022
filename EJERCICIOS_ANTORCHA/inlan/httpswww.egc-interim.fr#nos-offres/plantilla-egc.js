...
(function() {
    ...
    let container = Array.from(document.querySelectorAll('div[id="offre"]')); // Main job container
    ...
    if (out["pass_it"].totalJobs === 0) {
        ...
        for (let elem of container) {
            ...
            job.source_location = elem.querySelector('div[id="offre-bottom"] div:first-child span').textContent.trim();
            job.location = job.source_location;
            job.source_jobtype = elem.querySelector(`div[id="offre-bottom"] div:nth-child(2) span`).textContent.trim();
            job.temp = 'test';
            jobs.push(job);
        }
    ...
    container[out["pass_it"].cont].querySelector(`div[id="offre-bottom"] div:nth-child(3) button`).click();
    msg("Clicked Job = " + out["pass_it"].jobs[out["pass_it"].cont].title);
    //selector del boton al que se le hace click
    out.waitFor = 'div[id="Description"]';
    return out;
})();
//extract
(function() {
    ...
    var full_html = document.querySelector('div[id="Description"]');
