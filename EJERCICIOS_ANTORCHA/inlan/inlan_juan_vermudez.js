...
(function() {
    ...
    let container = Array.from(document.querySelectorAll('')); // Main job container
    ...
    if (out["pass_it"].totalJobs === 0) {
        ...
        for (let elem of container) {
            ...
            var dateAux = new Date(elem.querySelector('').textContent.trim());
            job.dateposted_raw = dateAux.toLocaleDateString("en-US");
            job.temp = '05/27/2022 Juan Bermudez';
            jobs.push(job);
        }
...
(function() {
    ...
    if (full_html) {
        ...
        job.url = document.querySelector('').href.trim();
