...
if (job.html.indexOf('www') > -1)  { job.html = job.html.replace(/(www?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \?=.-]*)*\/?/gi,"");  }
            ...
            // if(!job.source_salary) job.dateposted_raw=buscOcurrenciaHTML(elem,null,"",/\$|€|salary|SALARY|Salary/gmi,false);  
            // //*/ // jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj 
            ...
            let source_salary=[];  for (let li of document.querySelectorAll("p,li")) {
                if (/salary|payment|remuneration|salaire|paiement|salaire|salaris|uitbetaling|bezoldiging|Gehaltszahlung|Gehaltsvergütung/i.test(li.textContent) && /\$|¢|€/i.test(li.textContent)
                  && /\d|one|two|three|four|five|six|seven|eight|nine|ten/gi.test(li.textContent))
                  source_salary.push(li.textContent);
              }
              if(source_salary[0]!=undefined)job.source_salary=source_salary[0].replace(/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF]\[U+2728])/g, '').trim();
...
// if(job.dateposted_raw)job.dateposted_raw=validarFormatoFecha(job.dateposted_raw) ? addCero(job.dateposted_raw,"/"):undefined;
// if(job.dateclosed_raw)job.dateclosed_raw=validarFormatoFecha(job.dateclosed_raw) ? addCero(job.dateclosed_raw,"/"):undefined;
...
function validarFormatoFecha(campo) {
  ...
  for (const a of full_html.querySelectorAll('p')) { // Varios p
    ...
    if (text.search(/full time vacancy that/i) > -1) a.remove();
