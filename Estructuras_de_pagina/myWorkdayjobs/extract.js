...
/*
        ...
        var full_html1 = getDescription(job.url);
       ...
       // job.dateposted_raw=div.querySelectorAll('span.job-info')[0].textContent.trim();
        ...
        var json1=JSON.parse(html);
        //ORIGINAL:  var date=json.graph[1].datePublished.split("T").shift().split("-");
        var date1=json1.datePosted.split("T").shift().split("-");
        job.dateposted_raw=date[1]+"/"+date[2]+"/"+date[0];
        //var jobdescr=json.description;
           //   job.jobdesc   = cleanHTML(jobdescr);
        }
                ...
                */  
                ...
                if(i == array.length-1){                  
                  locations.map(loc =>{
                    ...
                    jobx.location = jobx.location.replace('- London - Saffron Hill)','Toronto, Ontario');
                    jobx.location = jobx.location.replace(/- Home Office ( - New York)/g,'Toronto, Ontario');
                    jobx.location = jobx.location.replace(/- Home Office - GBR/g,'Toronto, Ontario');
                    jobx.location = jobx.location.replace('- Home Office ( Toronto, Ontario','Toronto, Ontario');
                    jobx.location = jobx.location.replace('- Home Office ( - New York)','Toronto, Ontario');
