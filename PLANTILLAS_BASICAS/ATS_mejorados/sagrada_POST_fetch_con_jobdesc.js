...
(async () => {
  ...
  const resp = await fetch(`https://kohls.wd1.myworkdayjobs.com/wday/cxs/kohls/kohlscareers/job/${location.href.split('job/').pop()}`);
  const jsonDesc = await resp.json();

  full_html.innerHTML = jsonDesc.jobPostingInfo.jobDescription;

  if (jsonDesc.jobPostingInfo.jobDescription) {
