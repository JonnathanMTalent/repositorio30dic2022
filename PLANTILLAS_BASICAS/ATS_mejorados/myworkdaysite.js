...
myworkdaysite:
GET JOB DATA: NOT
Es necesario cambiar la URL del AJAX (peticion POST), 
Cambiar la data.
CAMBIAR EL DOMINIO INICIAL DE LA EMPRESA:
 job.url = //Formato para construir la url de cada job(descripcion). 
 let endpoint = //URL DE LA PETICION PARA CARGAR EL JSON DE LAS DESCRIPCIONES;
...
(function() {
    ...
    $.ajax({
      url : 'https://wd3.myworkdaysite.com/wday/cxs/havas/GroupExternalCareerSite/jobs', 
      "headers": {
      ...
      success : function(json){
        ...
        for (i in jobs) {
          ...
          //!!IMPORTANTEEEE:  Se debe cambiar la URL base del job para viajar a la descripcion.
          //!!IMPORTANTEEEE:  Se debe cambiar la URL base del enpoint que carga el JSON de la descripcion:
          job.url = "https://wd3.myworkdaysite.com/en-US/recruiting/havas/GroupExternalCareerSite"+ jobs[i].externalPath; 	                        
          ...
          let endpoint = "https://wd3.myworkdaysite.com/wday/cxs/havas/GroupExternalCareerSite"+ jobs[i].externalPath;
          job.location = jobs[i].locationsText;          
            ...
            job.html = removeTextAfter(job.html, "discrimination.", false);
