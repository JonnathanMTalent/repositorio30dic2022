...
TEMPLATES APOYO
Actualizado Actualizado el 15 de septiembre de 2022

0. SPIDER CONFIG - boo 3.1
1. EXTRACT
    Nota: Contiene la función: getDateFormat().
    Valida los meses en inglés, holandés, francés y portugués.   
...
// Boo 3.1
{
      ...
      var out = {};
          ...
          if(typeof html_jobs[x] =="function") continue;
            if(typeof html_jobs[x] =="number") continue;
              ...
              //job.dateposted_raw = getDateFormat(datePosted," ",0,1,2); // dateRaw, cut, dayPosition, monthPosition, yearPosition
    
              //let dateClosed     = elem.querySelector('dateClosedSelector').textContent.trim();
              //job.dateclosed_raw = getDateFormat(dateClosed,"/",0,1,2);
    
              job.temp  = "2022";
    
              //if(job.title.search(/open application/i)>-1){job.title = '';}
    ...
    // Variables que se pueden pasar: title, link, jobid, source_benefit
    //var job = pass_it["job"]; 
    
    
    // Validación de selector. 
    // Nota: Si no se valida el selector el código entrara en error y presentará flag html = 0 (no se publicará el job)
    ...
    */
    ...
    for (const a of document.querySelectorAll('p')) {
        if (a.innerText.search(/Benefits/)>-1){
    ...
    */
    ...
    return newHtml;
     ...
     //////////////////////////
    // 2.2 DESCRIPTION ARRAY // (Updated recently: August 27, 2022)
    /////////////////////////
    
          /* Si la descripción está en más de dos tags distintos se puede hacer un array. 
             Trabajar ese tipo de descripciones de esta forma ahorra tiempo en la limpieza.
    
            Este bloque sustituye las líneas: 
          ...
          */
          ...
          let descPart1 = document.querySelector("selector1");
          let descPart2 = document.querySelector("selector2");
          //let descPart3 = document.querySelector("selector3");
    ...
    // CASO A. SIMPLE
     ...
     let separator = '/';  
            ...
            if(job.location.indexOf(separator)>-1)
            
              var locs = job.location.split(separator); 
                for(w in locs){
    ...
    let selectorLoc  = 'li.location'; 
            ...
            if(elem.querySelector(selectorLoc) !== null){ // Validación del selector con multi-location
            
              var locs = elem.querySelectorAll(selectorLoc);
                for(w in locs){
    ...
    /////////////////////////////////// 
            ...
            desc.innerHTML = full_html
      
            for (const a of desc.querySelectorAll('a, button, script')) {
                if (a){ 
                  a.remove(); 
                } 
              }
    ...
    /////////
            ...
            desc.innerHTML = full_html
             ...
             for (const a of desc.querySelectorAll('a, button, script, style')) {
                if (a){ 
                  a.remove(); 
                } 
              }
    ...
    (function () {
        ...
        var url_base           = "";
        var main_loop_selector = ""; // Selector que se usa en el step extract para iterar el listado de jobs
    ...
    //////////////     
        ...
        let lastCharTitle = job.title.substr(job.title.length -1);
         if(lastCharTitle === "-"){job.title = job.title.slice(0,-1).trim();}
          ...
          // Split - city 2: Para limpiar las locaciones de los títulos
    ...
    */
    ...
    //////////////////
        ...
        for (const a of full_html.querySelectorAll('li')) {
          if (a.textContent.search(/experience|ervaring/i)>-1 && a.textContent.search(/[0-9]/g)>-1){
    ...
    ///////////////////////////////////////////
                   ...
                   let selectorFullHtml = nextUntil(elem, 'h3');
                   let full_html = "";
                   selectorFullHtml.forEach(data => {
                   full_html += data.innerHTML
                   })
    ...
    function nextUntil(elem, selector, filter) {
      ...
      // Remover el primer elemento de un location si el primer elemento es un lugar que pueda afectar el mapeo
      // 
                        if(job.location.indexOf(",")>-1){
                        var arr = job.location.split(",");
                            if(arr.length > 2){
                                arr.shift();
                                job.location = arr.join(",");
