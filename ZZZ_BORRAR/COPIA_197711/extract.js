...
(function() {
  ...
  $.ajax({
    ...
    success : function(json){
      ...
      for (i in jobs) {
        var job = {}; /*init*/
        job.title = jobs[i].title.instances[0].text.split("-")[0].trim();
        job.reqid = jobs[i].subtitles[1].id;//.instances[0].text;         
        ...
        if (job.location) {
          ...
          job.html = removeTextAfter(job.html, "Search Chair", true);
          job.html = removeTextAfter(job.html, "Contact Information", true);
          job.html = removeTextAfter(job.html, "The position will be jointly supervised", true);
          job.html = removeTextAfter(job.html, "https://", true);
          //job.html = removeTextAfter(job.html, "Required_Attachment", true);
          //job.html = removeTextAfter(job.html, "Required_Attachment", true);
            ...
            if(array[i].iconName){
              ...
              if(i == array.length-1){                  
                locations.map(loc =>{
                  ...
                  if(jobx.location.search(/Reno/gi) > -1)jobx.location = 'Reno, NV, US'
        ...
        if (!job.location || job.location == '') {
          ...
          job.location = "Reno, Nevada, US";
          if(job.title.length > 3)returnedJobs.push(job);
        }//end if job.location
      }
    },
