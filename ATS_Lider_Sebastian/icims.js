...
(function () {
    ...
    out["pass_it"] = {
        "offSet": 0,
        "limit": 0,
        "multiLink": ['https://globalhub-berryglobal.icims.com/', 'https://emeahub-berryglobal.icims.com/', 'https://latam-berryglobal.icims.com/'],
        "multiLinkCount": 0
    };
...
(function () {
    ...
    msg('URL: ' + out.pass_it.multiLink[out.pass_it.multiLinkCount] + '\nPage: ' + out.pass_it.offSet);
    $.ajax({
        url: out.pass_it.multiLink[out.pass_it.multiLinkCount] + 'jobs/search?in_iframe=1&pr=' + out.pass_it.offSet,
        headers: {
            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "accept-language": "en-GB,en;q=0.9,es-CO;q=0.8,es;q=0.7",
            "cache-control": "no-cache",
            ...
            for (var elem of html_jobs) {
                ...
                if (job.location.indexOf('|') > -1) {
                    ...
                    for (let auxLoc of array) {
                        var jobx = {};
                        jobx = { ...job }
                        jobx.location = auxLoc.split('-').reverse().map(loc => loc.trim()).join(', ').trim();
                        jobs.push(jobx);
                    }
                } else {
                    job.location = job.location.split('-').reverse().map(loc => loc.trim()).join(', ').trim();
                    jobs.push(job);
                }
            }
        },
