...
(function () {
  let out = {};
  out["pass_it"] = pass_it;
  let container = Array.from(document.querySelectorAll('')); // Main job container(selector de job)
  ...
  if (out["pass_it"].totalJobs === 0) {
    ...
    container[out["pass_it"].cont].querySelector('selector de boton descripcion').click();//Se le hace un queryselector cuando hay un boton para ir a la descripcion en caso contrario solo se deja el punto click
...
(function () {
  ...
  if (full_html) {
    job.url = window.location.href;// Se puede usar job,url en el before extrac o extrac(pero dejar solo uno) si presenta problemas comentar 
...
(function () {
  var out = {};
  out["pass_it"] = pass_it;
  var selector = ''; selector next page
  ...
  //out.pic=true;// toma una captura del jobsite(sirve para evaluar si entra al jobsite)
  // out.html=true;//Captura html (envia una copia del html jobsite)
